...
import '../../../core/services/hist_vehicule_service.dart';
import 'package:orbitnetmobileapp/screens/allScreens/navigation_drawer.dart';
import 'historique_vehicule_detail.dart';
class HistoriqueVehicule extends StatefulWidget {
  const HistoriqueVehicule({Key? key}) : super(key: key);

  @override
  State<HistoriqueVehicule> createState() => _HistoriqueVehiculeState();
}

class _HistoriqueVehiculeState extends State<HistoriqueVehicule> {
  ...
  Future<void> onRowTap(vin,immat,dateDeb,dateFin) async {
    List? ligneData = 
      await SearchVehiculeHistoryService.
        listHistVehic(context, vin,immat,dateDeb,dateFin,0,null,"");
    print("ligneData");
    print(ligneData);
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => VehicleHistoryPage(data: ligneData!),
      ),
    );
  }
              ...
              children: [
                ...
                Expanded(
                  child: TextField(
                    ...
                    onTap: () async {
                      FocusScope.of(context).requestFocus(FocusNode());
                      await _selectDate(context, dateFinController);
                    },
                  ),
                ),
              ],
